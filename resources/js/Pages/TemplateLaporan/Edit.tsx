...
import { ArrowLeft, Plus, Trash2, Camera, Type, Info } from 'lucide-react';
import DashboardLayout from '@/Layouts/DashboardLayout';
import type { CheckpointOption, MasterTemplateItem, TemplateFieldItem } from './types';
...
export default function TemplateLaporanEdit({ template, checkpoints = [] }: EditProps) {
    const { data, setData, put, processing, errors } = useForm<{
        ...
        applies_to_report_type: template.applies_to_report_type || 'movement',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/template-laporan/${template.id}`);
    };

    return (
        <DashboardLayout>
            <Head title={`Edit Template ${template.name} - GTD Logistics`} />
            ...
            <div className="w-full max-w-4xl mx-auto space-y-4">
                ...
                <div className="flex items-center gap-3 pb-3 border-b border-slate-200">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-[#06283A]">
                            Edit Master Template: {template.name}
                        </h1>
                        <p className="text-xs text-slate-500">
                            Perbarui konfigurasi form dan foto bukti untuk tahapan pengiriman
                        </p>
                    </div>
                </div>

                {/* ── Snapshot Immutability Notice ── */}
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-start gap-2.5 text-xs text-[#06283A]">
                    <Info size={16} className="text-blue-600 shrink-0 mt-0.5" />
                    <div>
                        <strong className="font-bold">Ketentuan Snapshot Historis:</strong>
                        <p className="text-slate-600 mt-0.5 leading-relaxed">
                            Perubahan pada master template ini <strong>hanya akan berlaku untuk sesi kargo baru</strong> yang dibuat setelahnya.
                            Sesi kargo yang sudah ada sebelumnya tetap mempertahankan snapshot historisnya secara permanen dan tidak akan terpengaruh.
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2">
                        ...
                        >
                            {processing ? 'Menyimpan...' : 'Simpan Perubahan Master Template'}
                        </button>
                    </div>
                </form>
            </div>
